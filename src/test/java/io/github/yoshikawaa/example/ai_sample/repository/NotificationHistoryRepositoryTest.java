...
class NotificationHistoryRepositoryTest {
    ...
    @Nested
    @DisplayName("特定顧客の通知履歴取得")
    class FindByRecipientEmailTest {

        @Test
        @DisplayName("findByRecipientEmail: 特定顧客の通知履歴を取得できる")
        void testFindByRecipientEmail() {
            // テストデータを挿入
            insertNotificationHistory("test@example.com", "PASSWORD_RESET", "Subject 1", "SUCCESS", LocalDateTime.now().minusDays(3));
            insertNotificationHistory("test@example.com", "ACCOUNT_LOCK", "Subject 2", "SUCCESS", LocalDateTime.now().minusDays(2));
            insertNotificationHistory("test@example.com", "ACCOUNT_UNLOCK", "Subject 3", "SUCCESS", LocalDateTime.now().minusDays(1));
            insertNotificationHistory("user@example.com", "PASSWORD_RESET", "Subject 4", "SUCCESS", LocalDateTime.now());

            // 特定顧客の通知履歴を取得
            List<NotificationHistory> histories = notificationHistoryRepository.findByRecipientEmail("test@example.com", null, null, 100);

            // 検証
            assertThat(histories).hasSize(3);
            assertThat(histories.get(0).getSentAt()).isAfter(histories.get(1).getSentAt());  // 新しい順
            assertThat(histories).allMatch(h -> h.getRecipientEmail().equals("test@example.com"));
        }

        @Test
        @DisplayName("findByRecipientEmail: 開始日時で絞り込みができる")
        void testFindByRecipientEmail_WithStartDate() {
            LocalDateTime now = LocalDateTime.now();
            // テストデータを挿入
            insertNotificationHistory("test@example.com", "PASSWORD_RESET", "Subject 1", "SUCCESS", now.minusDays(5));
            insertNotificationHistory("test@example.com", "ACCOUNT_LOCK", "Subject 2", "SUCCESS", now.minusDays(2));
            insertNotificationHistory("test@example.com", "ACCOUNT_UNLOCK", "Subject 3", "SUCCESS", now);

            // 3日前以降の通知履歴を取得
            LocalDateTime startDate = now.minusDays(3).withHour(0).withMinute(0).withSecond(0).withNano(0);
            List<NotificationHistory> histories = notificationHistoryRepository.findByRecipientEmail("test@example.com", startDate, null, 100);
        ...
        @Test
        @DisplayName("findByRecipientEmail: 終了日時で絞り込みができる")
        void testFindByRecipientEmail_WithEndDate() {
            LocalDateTime now = LocalDateTime.now();
            // テストデータを挿入
            insertNotificationHistory("test@example.com", "PASSWORD_RESET", "Subject 1", "SUCCESS", now.minusDays(5));
            insertNotificationHistory("test@example.com", "ACCOUNT_LOCK", "Subject 2", "SUCCESS", now.minusDays(2));
            ...
            List<NotificationHistory> histories = notificationHistoryRepository.findByRecipientEmail("test@example.com", null, endDate, 100);
        ...
        @Test
        @DisplayName("findByRecipientEmail: 期間指定で絞り込みができる")
        void testFindByRecipientEmail_WithDateRange() {
            LocalDateTime now = LocalDateTime.now();
            // テストデータを挿入
            insertNotificationHistory("test@example.com", "PASSWORD_RESET", "Subject 1", "SUCCESS", now.minusDays(10));
            insertNotificationHistory("test@example.com", "ACCOUNT_LOCK", "Subject 2", "SUCCESS", now.minusDays(5));
            insertNotificationHistory("test@example.com", "ACCOUNT_UNLOCK", "Subject 3", "SUCCESS", now.minusDays(3));
            insertNotificationHistory("test@example.com", "PASSWORD_RESET", "Subject 4", "FAILURE", now);
            ...
            List<NotificationHistory> histories = notificationHistoryRepository.findByRecipientEmail("test@example.com", startDate, endDate, 100);
        ...
        @Test
        @DisplayName("findByRecipientEmail: limit数で取得件数を制限できる")
        void testFindByRecipientEmail_WithLimit() {
            // テストデータを10件挿入
            for (int i = 0; i < 10; i++) {
                insertNotificationHistory("test@example.com", "PASSWORD_RESET", "Subject " + i, "SUCCESS", LocalDateTime.now().minusDays(i));
            }

            // limit=5で取得
            List<NotificationHistory> histories = notificationHistoryRepository.findByRecipientEmail("test@example.com", null, null, 5);
        ...
        @Test
        @DisplayName("findByRecipientEmail: 該当データがない場合は空リストを返す")
        void testFindByRecipientEmail_NoData() {
            // テストデータを挿入（別の顧客）
            insertNotificationHistory("user@example.com", "PASSWORD_RESET", "Subject", "SUCCESS", LocalDateTime.now());

            // 存在しない顧客の通知履歴を取得
            List<NotificationHistory> histories = notificationHistoryRepository.findByRecipientEmail("nonexistent@example.com", null, null, 100);
